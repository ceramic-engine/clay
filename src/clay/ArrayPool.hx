...
typedef ArrayPool = ceramic.ArrayPool
    ...
    public static function pool(size:Int):ArrayPool {
        ...
        if (size <= 10) {
            ...
            if (!didNotifyLargePool) {
                ...
                Log.warning('You should avoid asking a pool for arrays with more than 100000 elements (asked: $size) because it needs allocating a temporary one-time pool each time for that.');
            }
